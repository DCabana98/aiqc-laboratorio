...
#  Versión: 4.13 – Mejoras de seguridad, multi-usuario, auditoría
#  Deploy:  streamlit run app.py
#  Deps:    pip install streamlit plotly pandas numpy fpdf2 openpyxl
#           google-generativeai kaleido requests bcrypt
# ==============================================================
...
# ==============================================================
#  LOGGING — para pdf_txt y auditoría
# ==============================================================
logging.basicConfig(
    level=logging.INFO,
...
html, body, [data-testid="stAppViewContainer"], [data-testid="stAppViewBlockContainer"] {
    background-color: #F4F6F9 !important; color: #1C2B3A;
...
.stTabs [data-baseweb="tab"] {
    ...
    color: #64748B !important; font-weight: 500 !important; font-size: .875rem !important;
...
.kpi-card {
    background: #FFFFFF; border: 1px solid #E8EDF2; border-top: 3px solid #1A6FC4;
    border-radius: 14px; padding: 22px 20px 18px 20px; text-align: center;
...
.badge { display: inline-flex; align-items: center; gap: 6px; padding: 5px 14px; border-radius: 999px; font-size: .78rem; font-weight: 700; box-shadow: 0 1px 4px rgba(0,0,0,.10); }
...
.sync-pill { background:rgba(13,158,110,.12); border:1px solid rgba(13,158,110,.30); border-radius:10px; padding:10px 14px; font-size:.82rem; color:#34D399 !important; margin-top:8px; }
.audit-row { background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:8px 14px; margin-bottom:6px; font-size:.82rem; }
.role-admin { background:#EFF6FF; color:#1D4ED8; border:1px solid #BFDBFE; border-radius:6px; padding:2px 8px; font-size:.72rem; font-weight:700; }
.role-supervisor { background:#F0FDF4; color:#166534; border:1px solid #BBF7D0; border-radius:6px; padding:2px 8px; font-size:.72rem; font-weight:700; }
.role-tecnico { background:#FFFBEB; color:#92400E; border:1px solid #FDE68A; border-radius:6px; padding:2px 8px; font-size:.72rem; font-weight:700; }
...
BIORAD_KB = {
    "Glucosa": {
        "producto":"Liquichek Chemistry Control / Lyphochek Chemistry","grupo":"Bioquímica básica",
        "causas_comunes":["Evaporación del vial (apertura prolongada o temperatura elevada)",
            "Degradación glucolítica in vitro si el control no se procesa en <=2 h tras reconstitución",
            "Interferencia por hemólisis severa","Calibración desactualizada",
            "Contaminación cruzada con reactivos de otros analitos"],
        "acciones_1_3s":["No liberar resultados de pacientes hasta resolver la alarma",
            "Repetir el control con un NUEVO vial del mismo lote",
            "Si persiste: repetir con vial de LOTE DIFERENTE",
            "Verificar temperatura de almacenamiento (2-8 oC)",
            "Comprobar fecha de caducidad y tiempo desde reconstitución (max. 5 días)",
            "Recalibrar con estándar trazable IDMS","Documentar acción y responsable"],
        "acciones_warn":["Monitoreo estrecho","Revisar tendencia en Levey-Jennings",
            "Verificar temperatura del baño termostatizado","Registrar observación"],
    ...
    "Potasio (K+)": {
        "producto":"Liquichek Chemistry Control / Liquichek Electrolyte Plus","grupo":"Bioquímica básica",
        "causas_comunes":["Evaporación del vial (concentra el analito)",
            "Contaminación por EDTA","Hemólisis in vitro del control",
            "Interferencia del sodio en electrodos ISE","Temperatura incorrecta del módulo ISE"],
        "acciones_1_3s":["Verificar que el vial no lleva abierto más de 8 horas",
            "Repetir con vial nuevo","Revisar el electrodo ISE de potasio",
    ...
    "Sodio": {
        "producto":"Liquichek Chemistry Control / Liquichek Electrolyte Plus","grupo":"Bioquímica básica",
        "causas_comunes":["Pseudohiponatremia por lipemia severa en métodos de llama",
            "Dilución incorrecta del control","Electrodo ISE de sodio deteriorado"],
        "acciones_1_3s":["Repetir control con vial nuevo","Verificar volumen de reconstitución",
            "Revisar y limpiar el electrodo ISE de sodio","Recalibrar con NaCl trazable NIST"],
    ...
    "Creatinina": {
        "producto":"Liquichek Chemistry Control","grupo":"Bioquímica básica",
        "causas_comunes":["Interferencia por cromógenos de Jaffé (cefalosporinas, acetona, bilirrubina)",
            "Diferencia de método: Jaffé cinético vs enzimático","Calibración no trazable a IDMS"],
        "acciones_1_3s":["Confirmar valores del insert para TU método",
            "Cambiar a método enzimático si hay interferencia","Recalibrar con calibrador trazable IDMS (NIST SRM 967)"],
    ...
    "ALT (Transaminasa)": {
        "producto":"Liquichek Chemistry Control / Lyphochek Chemistry","grupo":"Enzimas hepáticas",
        "causas_comunes":["Temperatura de incubación incorrecta (muy sensible a T)",
        ...
        "acciones_1_3s":["Verificar temperatura del baño (37,0 oC +/- 0,1 oC)",
            "Repetir con vial nuevo","Comprobar que el reactivo contiene P-5-P activado",
            "Verificar longitud de onda del espectrofotómetro",
            "Si ALT y AST fallan: sospechar problema de temperatura","Recalibrar"],
        "acciones_warn":["Comprobar temperatura del módulo fotométrico","Verificar mezcla del vial",
            "Controlar absorbancia del blanco (no debe superar 1.5 AU)"],
        "causas_deriva":["Deterioro del NADH (sensible a luz UV)","Fluctuación de temperatura",
            "Cambio de lote sin ajuste de valores objetivo"],
        "estabilidad_biorad":"Reconstituido: 5 días a 2-8 oC (actividad decrece con el tiempo)",
    ...
    "AST": {
        "producto":"Liquichek Chemistry Control / Lyphochek Chemistry","grupo":"Enzimas hepáticas",
        "causas_comunes":["Hemólisis in vitro (AST eritrocitaria es 15x mayor que en plasma)",
            "Temperatura incorrecta (cada grado cambia la actividad ~7%)","P-5-P ausente o degradado"],
        "acciones_1_3s":["Inspeccionar el vial (hemólisis visible = color rosado)",
            "Repetir con vial nuevo sin hemólisis","Verificar temperatura (37,0 oC +/- 0,1 oC)"],
    ...
    "GGT": {
        "producto":"Liquichek Chemistry Control","grupo":"Enzimas hepáticas",
        "causas_comunes":["Temperatura incorrecta (+/- 0,5 oC)","pH fuera de rango (óptimo 7,9-8,2)",
            "Evaporación del substrato por mal sellado"],
        "acciones_1_3s":["Verificar temperatura del módulo fotométrico",
            "Comprobar pH del tampón","Repetir con vial nuevo y reactivo fresco"],
    ...
    "LDH": {
        "producto":"Liquichek Chemistry Control / Lyphochek Chemistry","grupo":"Enzimas hepáticas",
        "causas_comunes":["Hemólisis (LDH eritrocitaria es 160x mayor que en plasma)",
            "Temperatura critica: cada oC modifica la actividad ~8-10%",
            "Inhibición por exceso de piruvato en método inverso"],
        "acciones_1_3s":["Inspeccionar el vial (hemólisis = causa más frecuente)",
            "Repetir con vial nuevo sin hemólisis","Verificar temperatura (37,0 oC)"],
        "acciones_warn":["Verificar que el reactivo no tiene precipitados (NADH precipita en frio)",
    ...
    "Colesterol": {
        "producto":"Liquichek Lipid Control / Lyphochek Lipid","grupo":"Lípidos",
        "causas_comunes":["Diferencia de método: CHOD-PAP vs Abell-Kendall",
            "Interferencia por bilirrubina >5 mg/dL","Calibrador no trazable a NIST SRM 1951c"],
        "acciones_1_3s":["Verificar que los valores del insert corresponden a TU método",
            "Repetir con vial nuevo","Recalibrar con calibrador trazable a NIST SRM 1951c"],
        "acciones_warn":["Verificar mezcla del vial (colesterol puede precipitar)",
            "Confirmar que el blanco está dentro del rango de linealidad"],
    ...
    "Triglicéridos": {
        "producto":"Liquichek Lipid Control / Lyphochek Lipid","grupo":"Lípidos",
        "causas_comunes":["Glicerol endógeno libre en el control",
            "Interferencia por hemólisis (inhibe la peroxidasa)"],
        "acciones_1_3s":["Verificar si el insert especifica valores con/sin corrección por glicerol",
            "Repetir control con vial nuevo","Recalibrar con calibrador trazable a NIST SRM 1951c"],
    ...
    "HDL-Colesterol": {
        "producto":"Liquichek Lipid Control","grupo":"Lípidos",
        "causas_comunes":["Efecto matriz en métodos de precipitación directa",
            "Interferencia de VLDL elevadas","Calibración incorrecta del método homogéneo directo"],
        "acciones_1_3s":["Verificar valores del insert para TU método específico de HDL",
            "Repetir con vial nuevo","Recalibrar"],
        "acciones_warn":["Confirmar que el tipo de método coincide con los valores del insert",
            "Verificar integridad del blanco de HDL"],
    ...
    "TSH": {
        "producto":"Lyphochek Immunoassay Plus Control","grupo":"Inmunoensayo hormonal",
        "causas_comunes":["Anticuerpos heterófilos (HAMA) en ensayos sandwich",
            "Degradación por ciclos de congelación/descongelación",
            "Variabilidad inter-ensayo elevada (CV% tipico 5-8%)"],
        "acciones_1_3s":["Verificar que el control está asignado a TU plataforma",
            "Repetir con vial nuevo","Revisar número de ciclos de congelación (max. 3)",
            "Verificar calibración del inmunoensayo"],
        "acciones_warn":["Revisar número de lote del reactivo vs calibración activa",
            "Verificar que el cartucho no está próximo a caducidad"],
    ...
    "T4 Libre (FT4)": {
        "producto":"Lyphochek Immunoassay Plus Control","grupo":"Inmunoensayo hormonal",
        "causas_comunes":["Interferencia por proteínas de unión (TBG, albúmina)",
            "Dilución incorrecta del control liofilizado","Variabilidad entre plataformas"],
        "acciones_1_3s":["Confirmar que los valores objetivo son específicos para TU analizador",
            "Repetir con vial nuevo reconstituido correctamente","Recalibrar el inmunoensayo"],
        "acciones_warn":["Comprobar el volumen de reconstitución exacto",
            "Mezclar por inversión suave (no vortex)"],
    ...
    "Hemoglobina": {
        "producto":"Lyphochek Hematology / Liquichek Hematology","grupo":"Hematología",
        "causas_comunes":["Envejecimiento del control (eritrocitos se fragmentan)",
            "Temperatura de almacenamiento incorrecta","Variabilidad entre analizadores"],
        "acciones_1_3s":["Verificar fecha de caducidad del vial abierto (5-7 días)",
            "Repetir con vial nuevo","Recalibrar con material del fabricante del analizador"],
        "acciones_warn":["Verificar temperatura (2-8 oC, NO congelar)",
            "Invertir suavemente 8-10 veces antes de analizar",
            "Estabilización de 15 min a T ambiente tras nevera"],
        "causas_deriva":["Fragmentación progresiva de eritrocitos","Variación del canal HGB por suciedad"],
    ...
    "Calcio": {
        "producto":"Liquichek Chemistry Control","grupo":"Bioquímica básica",
        "causas_comunes":["Interferencia por EDTA (quelante del calcio)",
            "pH del control fuera de rango","Interferencia por magnesio elevado","Evaporación del vial"],
        "acciones_1_3s":["Descartar contaminación con EDTA","Repetir con vial nuevo",
            "Verificar pH del reactivo de o-cresolftaleína","Recalibrar con calibrador trazable NIST SRM 956c"],
...
# ==============================================================
#  BASE DE CONOCIMIENTO COBAS 8000
# ==============================================================
COBAS_8000_KB = """
...
- Módulos: cobas ISE (electrólitos ISE), cobas c 701/c 702/c 502 (fotométrico), cobas e 602 (inmunoensayo ECL).
- Funciona 24 h/día. Temperatura ambiente: 18–32 °C. Humedad: 30–85 %.
- Intervalo de calibración ISE: 24 horas obligatorio para todos los tests ISE.

## MÓDULOS Y ANALITOS
- cobas ISE: Na+, K+, Cl-, Ca++ ionizado. Tecnología electrodo selectivo de iones.
- cobas c 701/c 702/c 502: Fotométrico. Glucosa, ALT, AST, GGT, LDH, Creatinina, Colesterol, Triglicéridos, HDL, Calcio.
- cobas e 602: Inmunoensayos ECL. TSH, T4 Libre, etc.

## MANTENIMIENTO DIARIO — ANTES DE PROCESAR MUESTRAS
1. Encender y verificar visualmente ausencia de fugas durante inicialización.
2. Procesar la BANDEJA VERDE (green wash rack): ~20 min. Obligatoria calibración ISE completa tras el lavado.
3. Limpiar pipeta de muestra (exterior): Standby → abrir cubierta → cubrir recipientes dilución ISE con papel → limpiar pipeta.
4. Limpiar salidas puertos vaciado ISE: gasa + agua desionizada al finalizar el análisis diario.
5. Calibración y QC ANTES de procesar muestras.
6. Actualizar parámetros (e-barcodes).

## PUNTOS CRÍTICOS QC — MÓDULO ISE
- Temperatura agua de incubación ISE: 37 °C.
- Cristales blancos en puertos vaciado → reduce aislamiento electrodos.
- Cubiertas ISE deben estar COLOCADAS para cualquier medición.

## PUNTOS CRÍTICOS QC — MÓDULOS FOTOMÉTRICOS c 701/c 702
- Longitud de onda NADH (ALT, AST, LDH): 340 nm.
- Temperatura baño: 37,0 °C ± 0,1 °C.

## CALIBRACIÓN
- Calibrar todos los tests ISE cada 24 horas (obligatorio).
- Trazabilidad: creatinina → NIST SRM 967; colesterol → NIST SRM 1951c.

## TROUBLESHOOTING RÁPIDO
| Síntoma | Causa probable | Acción cobas 8000 |
|---|---|---|
| Error QC ISE (K+, Na+, Cl-) | Electrodo desgastado / cristales en puerto / temperatura incorrecta | Limpiar puerto vaciado, revisar electrodo, bandeja verde + calibrar |
| Error QC enzimas (ALT/AST/LDH) | Temperatura baño incorrecta / longitud de onda 340 nm desviada | Verificar temperatura 37,0 °C, verificar fotómetro |
| Alarma Interlock | Cubierta abierta durante operación | Cerrar cubierta, reinicializar módulo |
| Calibración ISE no válida | Bandeja verde no procesada / electrodo / temperatura | Procesar bandeja verde → calibrar ISE |
"""


# ==============================================================
#  1. GOOGLE GEMINI — SYSTEM PROMPT v4.13
# ==============================================================
...
GEMINI_SYSTEM = (
    ...
    "Los datos de QC se generan en OpenLab (Agilent) y se sincronizan automáticamente con la app. "
    ...
    "según el manual del cobas 8000. "
    "La regla R-4s indica ERROR ALEATORIO entre niveles — NO recalibrar como primer paso. "
    "NUNCA inventes datos, resultados ni análisis. Usa SOLO los datos del bloque === DATOS REALES ===. "
    "Si no hay datos para un analito, dilo explícitamente. "
    "Respondes siempre en español. Temas técnicos: tono técnico y conciso con Markdown. "
    "Conversación general: tono cercano y natural. Z-Score: Z = (x - media) / SD."
)
...
# ==============================================================
#  2. BASE DE DATOS — PATH PERSISTENTE + ESQUEMA v4.13
# ==============================================================

def get_db_path() -> str:
    """
    MEJORA: Ruta persistente fuera del directorio de trabajo.
    En Streamlit Cloud usa /tmp (persiste durante la sesión del contenedor).
    En local usa el directorio home del usuario para sobrevivir reinicios.
    Configurable via secrets: [db] path = "/ruta/personalizada/aiqc.db"
    """
    custom = st.secrets.get("db", {}).get("path", "")
    if custom:
        return custom
    # En producción Streamlit Cloud, /tmp es el más estable disponible
    # En local, usar el home del usuario
...
def init_db():
    """
    MEJORA: Esquema extendido con tabla de usuarios (roles + bcrypt)
    y tabla de auditoría para trazabilidad completa.
    """
    con = sqlite3.connect(DB_PATH, check_same_thread=False)
    con.execute("PRAGMA journal_mode=WAL")  # Mejor rendimiento concurrente

    # Tabla original de acciones QC (sin cambios de estructura)
    con.execute("""
    ...
    # NUEVA: Tabla de usuarios con roles y contraseñas hasheadas (bcrypt)
    con.execute("""
    ...
    # NUEVA: Tabla de auditoría — registro de quién hizo qué y cuándo
    con.execute("""
...
def _seed_admin(con):
    ...
    La contraseña se lee de secrets.toml o usa 'admin2024' como fallback.
    En producción SIEMPRE configura [auth] admin_password en secrets.toml.
    """
    existe = con.execute("SELECT COUNT(*) FROM usuarios").fetchone()[0]
    if existe == 0:
...
# ==============================================================
#  3. FUNCIONES DE AUTENTICACIÓN (bcrypt)
# ==============================================================

def verificar_password(password: str, password_hash: str) -> bool:
    """MEJORA: Verificación segura con bcrypt (tiempo constante, resistente a fuerza bruta)."""
    try:
...
def hash_password(password: str) -> str:
    """Genera hash bcrypt con coste por defecto (12 rondas)."""
    return bcrypt.hashpw(password.encode(), bcrypt.gensalt()).decode()

def login_usuario(con, username: str, password: str):
    """
    MEJORA: Login con bcrypt + actualiza último acceso + registra en auditoría.
...
def registrar_auditoria(con, usuario: str, accion: str, detalle: str = ""):
    """MEJORA: Registra todas las acciones relevantes para trazabilidad ISO 15189."""
    try:
        con.execute("""
...
def tiene_permiso(rol_usuario: str, rol_requerido: str) -> bool:
    """
    Jerarquía de roles: admin > supervisor > tecnico.
    Un rol superior siempre puede hacer lo que hace uno inferior.
    """
...
# ==============================================================
#  4. INTERFAZ DE LOGIN (multi-usuario)
# ==============================================================

def render_login(con):
    """MEJORA: Login real contra BD de usuarios con bcrypt."""
...
# ==============================================================
#  5. SQLITE — FUNCIONES DE ACCIONES
# ==============================================================
...
def save_accion(con, clave, hecha, usuario="sistema"):
    """MEJORA: Guarda acción con usuario responsable + registra en auditoría."""
    con.execute(
        "INSERT OR REPLACE INTO acciones VALUES (?, ?, datetime('now'), ?)",
        (clave, int(hecha), usuario)
    )
    con.commit()
    estado_txt = "COMPLETADA" if hecha else "PENDIENTE"
    registrar_auditoria(con, usuario, f"ACCION_{estado_txt}", clave)


# ==============================================================
#  6. INICIALIZACIÓN DE BD Y SESIÓN
# ==============================================================
...
# Auth — pasar por login si no autenticado
...
# ==============================================================
#  7. DATOS DEMO — MEJORA: ttl en lugar de ref_date como hack
# ==============================================================

@st.cache_data(ttl=3600, show_spinner=False)  # MEJORA: ttl=3600s en lugar de pasar fecha como string
def build_demo():
    """Cache con TTL real de 1 hora. Se recalcula automáticamente al expirar."""
    np.random.seed(2026)
    ...
    for analito, niveles in NIVELES_DEMO.items():
        for nivel_cod, (media, sd) in niveles.items():
            for i, d in enumerate(dates):
                ...
                rows.append({
                    "Fecha": d, "Analito": analito, "Nivel": nivel_cod,
                    "Valor": round(np.random.normal(media + drift, sd * 0.85), 3),
                    "Media_Objetivo": media, "SD_Objetivo": sd,
...
# ==============================================================
#  9. GITHUB SYNC (OpenLab → GitHub → App)
# ==============================================================
def leer_csv_github() -> tuple:
    try:
        ...
        if not all([usuario, repo, archivo]):
            return None, "Faltan datos en secrets.toml — sección [github]: usuario, repo, archivo."
        ...
        if r.status_code == 401:
            return None, "Token de GitHub inválido o sin permisos de lectura."
        ...
        try:
            ...
            df_raw = pd.read_csv(io.BytesIO(contenido), sep=None, engine="python", encoding="utf-8-sig")

        df, err = normalizar_df(df_raw)
        if df is None:
            return None, f"CSV descargado pero con formato incorrecto: {err}"
...
def _auto_refresh_github():
    """
    MEJORA: El auto-refresco ahora se ejecuta SIEMPRE que hay configuración GitHub,
    sin depender de que df_github ya exista en session_state.
    Antes: `if tiene_config and st.session_state.get("df_github") is not None`
    → bloqueaba el primer refresco porque df_github era None en la primera carga.
    Ahora: solo comprueba si han pasado los minutos de intervalo.
    """
    INTERVALO_MIN = 60
    cfg_gh = st.secrets.get("github", {})
    if not (cfg_gh.get("usuario") and cfg_gh.get("repo") and cfg_gh.get("archivo")):
        return  # Sin configuración GitHub, no hacer nada

    ahora  = datetime.now()
    ultima = st.session_state.get("github_last_sync")

    if ultima is None or (ahora - ultima).total_seconds() > INTERVALO_MIN * 60:
        ...
        if df_gh is not None:
            ...
            logger.info(f"Auto-refresco GitHub completado: {msg}")
...
def render_r4s_alert(r4s):
    st.markdown(f"""<div style="background:#FFFAFA;border:1.5px solid #FECACA;border-left:5px solid #E53E3E;
        ...
        <div style="font-size:13px;color:#7F1D1D;line-height:1.7">
            <b>{r4s['analito']}</b>: nivel <b>{r4s['label_a']}</b> Z=<b>{r4s['z_a']:+.2f}sigma</b>
            vs nivel <b>{r4s['label_b']}</b> Z=<b>{r4s['z_b']:+.2f}sigma</b><br>
            Diferencia: <b>{r4s['diferencia']:.2f}sigma</b> (umbral R-4s: >= 4.0sigma) — indica <b>error aleatorio</b>.
    ...
    with c1:
        st.markdown(f"""<div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:10px;padding:14px 16px;">
            ...
            <div style="font-size:12px;color:#7F1D1D;margin-top:4px">Media:{r4s['media_a']} SD:{r4s['sd_a']} Z=<b>{r4s['z_a']:+.2f}</b></div>
    ...
    with c2:
        st.markdown(f"""<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:14px 16px;">
            ...
            <div style="font-size:12px;color:#1E40AF;margin-top:4px">Media:{r4s['media_b']} SD:{r4s['sd_b']} Z=<b>{r4s['z_b']:+.2f}</b></div>
        </div>""", unsafe_allow_html=True)
...
def calcular_ewma(z_scores, lam=0.20):
    n = len(z_scores)
    if n == 0:
        return {"ewma":[],"estados":[],"ultimo_ewma":0,"sigma_ewma":0,"inicio_deriva":None,"lim_warn":0,"lim_act":0}
    sigma_ewma = (lam / (2 - lam)) ** 0.5
    lim_warn = 2.0 * sigma_ewma; lim_act = 3.0 * sigma_ewma
...
def calcular_cusum(z_scores, k=0.5, h=5.0):
    n = len(z_scores)
    if n == 0:
        return {"cusum_pos":[],"cusum_neg":[],"alarma_any":[],"primera_alarma":None,"max_cp":0,"max_cm":0,"tipo_deriva":None}
    ...
    return {"cusum_pos": cp, "cusum_neg": cm, "alarma_any": alarma,
            "primera_alarma": primera, "max_cp": round(max(cp), 3),
            "max_cm": round(max(cm), 3), "tipo_deriva": tipo}
...
def build_ewma_figure(fechas, z_scores, ewma_r, analito, nivel):
    ...
    for y, col, dash, name in [
        (lim_a, "#E53E3E", "dot", f"+3sigma ({lim_a:.3f})"), (-lim_a, "#E53E3E", "dot", "-3sigma"),
        (lim_w, "#F59E0B", "dash", f"+2sigma ({lim_w:.3f})"), (-lim_w, "#F59E0B", "dash", "-2sigma"),
    ...
    for estado in ["Verde","Ámbar","Rojo"]:
        ...
        fig.add_trace(go.Scatter(
            x=[fechas[i] for i in idx], y=[ewma_r["ewma"][i] for i in idx],
    ...
    fig.update_layout(template="plotly_white",
        ...
        yaxis=dict(gridcolor="#F1F5F9", linecolor="#E2E8F0", title="EWMA (sigma)"),
        height=360, margin=dict(l=10, r=140, t=50, b=40),
...
def build_cusum_figure(fechas, cusum_r, analito, nivel):
    nivel_label = NIVELES.get(nivel, NIVELES["N"])["label"]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=fechas, y=cusum_r["cusum_pos"], name="C+ (subida)",
        marker_color=["#E53E3E" if a else "#93C5FD" for a in cusum_r["alarma_any"]], opacity=0.85))
    fig.add_trace(go.Bar(x=fechas, y=[-v for v in cusum_r["cusum_neg"]], name="C- (bajada)",
    ...
    fig.update_layout(template="plotly_white", barmode="overlay",
        ...
        paper_bgcolor="#FFFFFF", plot_bgcolor="#FAFBFC", font=dict(color="#475569", family="Inter"),
        xaxis=dict(gridcolor="#F1F5F9", linecolor="#E2E8F0", tickformat="%d %b", title="Fecha"),
        yaxis=dict(gridcolor="#F1F5F9", linecolor="#E2E8F0", title="CUSUM (sigma)"),
        height=320, margin=dict(l=10, r=140, t=50, b=40),
        legend=dict(orientation="h", y=1.08, x=1, xanchor="right"))
    return fig
...
def build_lj_figure(df_series, analito, nivel):
    ...
    fig = go.Figure()
    for y0, y1, col in [
        (m+2*sd, m+3*sd, "rgba(229,62,62,.10)"), (m-3*sd, m-2*sd, "rgba(229,62,62,.10)"),
    ...
    for y_v, color, width, dash, name in [
        ...
        (m+sd, "#94A3B8", 1.0, "dash", "+1 SD"), (m-sd, "#94A3B8", 1.0, "dash", "-1 SD"),
...
def generar_csv(df_log, acciones_db, claves_log):
    ...
    export["Nivel_Label"] = export.get("_nivel_label", export["Nivel"].map(
        lambda n: NIVELES.get(n, NIVELES["N"])["label"]))
...
# ==============================================================
#  15. PDF — pdf_txt CON LOGGING DE CARACTERES PROBLEMÁTICOS
# ==============================================================

# Tabla de sustituciones para latin-1
_PDF_REEMPLAZOS = {
    "—":"-","–":"-","−":"-","±":"+/-","×":"x","÷":"/","sigma":"sigma","μ":"u",
    "→":"->","↑":"(+)","↓":"(-)","≥":">=","≤":"<=","≠":"!=","≈":"~","°":"o",
    "\u2019":"'","\u201C":'"',"\u201D":'"',"🔴":"[ROJO]","🟡":"[AMBAR]",
...
def pdf_txt(s: str) -> str:
    """
    MEJORA: En lugar de silenciar errores con errors='replace',
    ahora se detectan y loggean los caracteres que no pueden codificarse en latin-1.
    Esto permite identificar y añadir sustituciones a _PDF_REEMPLAZOS en el futuro.
    """
    for orig, repl in _PDF_REEMPLAZOS.items():
        s = s.replace(orig, repl)

    # Detectar y loggear caracteres problemáticos ANTES de la conversión final
    problematicos = []
    for i, ch in enumerate(s):
        try:
            ch.encode("latin-1")
        except UnicodeEncodeError:
            problematicos.append((i, ch, ord(ch)))

    if problematicos:
        # Loggear para que el desarrollador pueda añadir sustituciones
        for pos, ch, codepoint in problematicos[:10]:  # máximo 10 para no saturar el log
            logger.warning(f"pdf_txt: carácter no latin-1 en pos {pos}: '{ch}' (U+{codepoint:04X}) — añadir a _PDF_REEMPLAZOS")

    # Conversión final con reemplazo explícito (ahora consciente de los problemas)
...
def generar_pdf(df_all, analitos, fuente, f_min=None, f_max=None, lab_nombre="LAB. CENTRAL"):
    ...
    sec("2. Semaforo de Estado por Analito y Nivel")
    ...
    sec("6. Guia Bio-Rad de Acciones Correctivas")
    alarmas = set()
    for an in analitos:
        for niv in niveles_disponibles:
            sub = evaluar_westgard(df_all[(df_all["Analito"]==an)&(df_all["Nivel"]==niv)].copy())
...
_PALABRAS_TECNICAS_QC = {
    ...
    "alt","ast","creatinina","colesterol","hemoglobina","r-4s","r4s","1_3s","2_2s",
    "4_1s","10_x","bio-rad","biorad","lote","nivel","normal","patológico","patologico",
    "informe","pdf","csv","exportar","incidencia","laboratorio","analisis","análisis",
    "muestra","paciente","clinico","clínico","cobas","openlab","ise","fotométrico",
    "fotometrico","pipeta","bandeja","verde","calibracion","calibración","mantenimiento",
    "interlock","electrodo","absorbancia","enzima",
}
...
def _necesita_datos_qc(pregunta):
    ...
    if len(palabras) <= 3 and not any(t in texto for t in _PALABRAS_TECNICAS_QC): return False
    if not any(t in texto for t in _PALABRAS_TECNICAS_QC): return False
    return True
...
def ia_responde_gemini(pregunta, historial, df_all, analitos_ls, f_min, f_max):
    ...
    if _necesita_datos_qc(pregunta):
        ...
        for an in analitos_ls:
            for niv in niveles_disponibles:
                sub = evaluar_westgard(df_all[
                    (df_all["Analito"]==an) & (df_all["Nivel"]==niv) &
        ...
        for an in analitos_ls:
            ...
            if r4s:
                r4s_lines.append(f"  [R-4s] {an}: {r4s['label_a']} Z={r4s['z_a']:+.2f} vs {r4s['label_b']} Z={r4s['z_b']:+.2f}")
...
# Auto-refresco GitHub (MEJORA: se ejecuta siempre, no condicionado a df_github)
...
with st.sidebar:
    ...
    with tab_src2:
        ...
        if not tiene_config:
            ...
            if ultima_sync:
                st.markdown(f'<div class="sync-pill">☁️ <b>OpenLab · GitHub</b><br>{msg_gh}</div>',
                            unsafe_allow_html=True)
            else:
...
# ==============================================================
df_raw    = df_all[
    (df_all["Analito"]==analito) & (df_all["Nivel"]==nivel_activo) &
    (df_all["Fecha"]>=pd.Timestamp(f_min)) & (df_all["Fecha"]<=pd.Timestamp(f_max))
].copy()
...
with tab_dash:
    if df_series.empty or ultima is None:
        ...
        if r4s_result:
            st.markdown("<br>", unsafe_allow_html=True); render_r4s_alert(r4s_result)
...
with tab_sigma:
    ...
    if not sigma_data:
        ...
        st.write(pd.DataFrame([{
            "Analito":d["analito"],"Nivel":d["nivel_label"],"N":d["n"],"Media":d["media"],
            "SD":d["sd"],"CV%":f"{d['cv_pct']}%","Sesgo%":f"{d['sesgo_pct']}%",
...
with tab_chat:
    ...
    if "messages" not in st.session_state:
        st.session_state["messages"] = [{"role":"assistant","content":(
            ...
            "Tengo acceso a los datos de OpenLab en tiempo real (vía GitHub), "
            "la base de conocimiento Bio-Rad y el manual del **cobas® 8000**.\n\n"
            "Puedo responder sobre alarmas, EWMA, CUSUM, R-4s, mantenimiento del analizador "
            "o cualquier otra consulta del laboratorio."
        )}]
...
# ── TAB 6: REGISTRO + CSV + PDF ──────────────────────────────
with tab_log:
    col_ttl, col_csv, col_pdf = st.columns([3,1,1])
    with col_ttl:
        st.markdown("### 📝 Registro de Incidencias y Trazabilidad")
        st.caption(f"Acciones persistentes en SQLite · Fuente activa: {data_src} · Usuario: {usuario_actual}")
    ...
    with col_pdf:
        ...
        if st.button("📄 Descargar PDF", use_container_width=True, type="primary"):
            if not tiene_permiso(rol_actual, "supervisor"):
                st.error("❌ Solo supervisores y administradores pueden generar informes PDF.")
            else:
    ...
    if df_log.empty:
        ...
        for idx, row in df_log.iterrows():
            ...
            if nuevo != prev:
                save_accion(db_con, key, nuevo, usuario=usuario_actual)  # MEJORA: pasa usuario
...
with tab_usuarios:
    ...
    if not tiene_permiso(rol_actual, "admin"):
        ...
        for uid, uname, rol_u, nombre_u, activo_u, creado, ultimo in users:
            ...
            with c4:
                if uname != usuario_actual:  # No puede desactivarse a sí mismo
            ...
            if submitted:
                if not nuevo_user or not nuevo_pwd:
                    ...
                    if existe:
                        ...
                        db_con.commit()
                        registrar_auditoria(db_con, usuario_actual, "USUARIO_CREADO",
                                            f"{nuevo_user} [{nuevo_rol}]")
        ...
        audit_rows = db_con.execute("""
            SELECT ts, usuario, accion, detalle FROM auditoria
            ORDER BY id DESC LIMIT ?
        """, (int(n_registros),)).fetchall()
        ...
        if not audit_rows:
            ...
            for ts, aud_user, accion, detalle in audit_rows:
                color_accion = {
                    "LOGIN_OK":"#0D9E6E","LOGIN_FALLIDO":"#E53E3E","LOGOUT":"#94A3B8",
                    "EXPORT_PDF":"#1A6FC4","SYNC_GITHUB":"#6B5CA5",
                }.get(accion, "#475569")
                st.markdown(
...
with tab_cfg:
    ...
    with col_lote1:
        lote_input = st.text_input("Número de lote", value=st.session_state["cfg_lote"],
